...
export async function runFileStreamChecks(check: (name: string, run: () => Promise<void>) => Promise<void>) {
  ...
  await check("files.trash", async () => {
    const path = `${await files.getDirectory("temp")}/spark-recycle-probe-${Date.now()}.txt`;
    await files.writeText(path, "Disposable Spark Trash/Recycle Bin acceptance file. Restore through the OS file manager.");

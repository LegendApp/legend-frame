...
import { DragDropView, type DropEvent, type DragOverEvent } from "@legend-apps/desktop/drag-drop";
import { writeText } from "@legend-apps/desktop/files";
import { openWindow, closeWindow } from "@legend-apps/desktop/windows";
...
const source = { data: { "application/x-legend-test-item": '{"id":42}' } };
const types = ["application/x-legend-test-item"];
        ...
        if (driver) {
          await check("AppKit overlay is transparent, borderless, nonactivating and status-level", async () => {
            try {
              ...
              const info = JSON.parse(await driver.call("overlayInfo", JSON.stringify({ identifier: "legend.foundation-overlay" })));

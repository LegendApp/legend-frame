...
The package manager installs the workspace dependencies and applies the checked-in desktop library
...
machine with `npx --no-install frame sdk register <runtime-directory>`.
...
Subsequent `npm run macos` / `npm run windows` starts Metro and opens that runtime.
...
platforms preserves them. Generated native projects and `.frame` state stay local
...
From the repository root, `npm run kitchen-sink` is a shortcut for this app's `dev`
script. `npm run kitchen-sink:prepare` deliberately packs the SDK and creates a
separate consumer under `.frame/examples/KitchenSinkPackaged` for integration tests.
It never overwrites this app's manifest, identity, or native projects.

External-library adapter changes use `npm run sync:workspace-patches`, then
`npm run postinstall`. That maintainer command derives the install patches from

...
import { getDirectory, writeText, remove, exists } from "@legend-apps/desktop/files";
...
const worker = "legend-background-proof";
...
export default function App(props: { launchArguments?: string[] }) {
  ...
  const reportIndex = args.indexOf("--legend-runtimes-report");
  ...
  async function run() {
    ...
    try {
      if (reportPath && args.includes("--legend-runtimes-reload")) {

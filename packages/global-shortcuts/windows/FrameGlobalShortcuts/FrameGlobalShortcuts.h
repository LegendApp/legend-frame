...
namespace winrt::FrameGlobalShortcuts {
...
struct Hotkeys {
  ...
  void EnsureWindow() {
    if (hwnd) return;
    static const wchar_t *name = L"FrameGlobalShortcuts";
...
REACT_MODULE(FrameGlobalShortcuts, L"NativeDesktopGlobalShortcuts")
struct FrameGlobalShortcuts {
  ...
  ~FrameGlobalShortcuts() { auto current = std::move(state); if (current->context) current->context.UIDispatcher().Post([current]() { current->Close(); }); }
  REACT_METHOD(call)

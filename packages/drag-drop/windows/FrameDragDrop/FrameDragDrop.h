...
struct DragView : implements<DragView, Windows::Foundation::IInspectable> {
  ...
  Point Local(Point point) {
    ...
    auto rect = strong.as<IFrameComponentGeometry>()->FrameClientRect(); auto scale = strong.LayoutMetrics().PointScaleFactor; if (scale <= 0) scale = 1;
  ...
  fire_and_forget Start(Microsoft::UI::Input::PointerPoint point, Composition::Input::Pointer pointer) {
    ...
    try {
      ...
      data.SetData(L"application/x-frame-drag", box_value(to_hstring(source)));
...
inline com_ptr<DragView> RootState::Target(Point point) {
  ...
  float x = 0, y = 0; auto tag = strong.as<IFrameComponentGeometry>()->FrameHitTest(point.X, point.Y, &x, &y);
...
inline IAsyncOperation<Json::JsonObject> Payload(Drag::DragInfo info, Point point, Json::JsonArray types, Operation operation) {
  ...
  if (data.Contains(L"application/x-frame-drag")) result = Json::JsonObject::Parse(unbox_value<hstring>(co_await data.GetDataAsync(L"application/x-frame-drag")));
  else {
...
inline std::shared_ptr<RootState> ForRoot(Composition::RootComponentView const &root) {
  ...
  root.as<IFrameComponentGeometry>();

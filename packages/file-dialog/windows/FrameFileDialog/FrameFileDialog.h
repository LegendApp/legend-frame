...
namespace winrt::FrameFileDialog {
...
inline void Write(std::string const &value, std::string const &contents) {
  ...
  auto temporary = file; temporary += std::wstring(L".frame-") + id;
  try {
...
REACT_MODULE(FrameFileDialog, L"NativeFileDialog")
struct FrameFileDialog {

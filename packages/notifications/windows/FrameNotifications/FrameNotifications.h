...
namespace winrt::FrameNotifications {
...
struct NotificationsState : std::enable_shared_from_this<NotificationsState> {
  ...
  static constexpr wchar_t const *group = L"Frame";
  void Ensure() {
    if (notifier) return;
    project = Env(L"FRAME_PROJECT_ID"); if (project.empty()) throw hresult_invalid_argument(L"Missing project identity");
    auto key = Hash(project); appId = L"Frame." + key;
    ...
    for (auto name : {L"FRAME_PROJECT_ID", L"FRAME_PROJECT_NAME", L"FRAME_PROJECT_VERSION", L"FRAME_WINDOW_CONFIG", L"FRAME_SESSION_FILE"}) launch.SetNamedValue(name, Json::JsonValue::CreateStringValue(Env(name)));
    auto arguments = L"--frame-context=" + std::wstring(Windows::Foundation::Uri::EscapeComponent(launch.Stringify()));
    Registry(L"Software\\Classes\\CLSID\\" + classId + L"\\LocalServer32", nullptr, L"\"" + std::wstring(executable) + L"\" " + arguments + L" --frame-toast-server");
    Registry(L"Software\\Classes\\AppUserModelId\\" + appId, L"DisplayName", Env(L"FRAME_PROJECT_NAME"));
    ...
    check_hresult(link.as<IPersistFile>()->Save((folder / (L"Frame-" + key + L".lnk")).c_str(), TRUE));
    ...
    auto registration = L"Software\\Frame\\" + appId;
    ...
    if (RegGetValueW(HKEY_CURRENT_USER, registration.c_str(), L"ToastRegistered", RRF_RT_REG_DWORD, nullptr, &registered, &bytes) != ERROR_SUCCESS || !registered) {
      ...
      Toast::ToastNotification bootstrap(xml); bootstrap.SuppressPopup(true); bootstrap.Tag(L"bootstrap"); bootstrap.Group(L"FrameBootstrap"); bootstrap.ExpirationTime(clock::now() + std::chrono::seconds(15));
      candidate.Show(bootstrap); Toast::ToastNotificationManager::History().Remove(L"bootstrap", L"FrameBootstrap", appId);
  ...
  void Dismiss(hstring const &encoded) {
    ...
    EnumWindows([](HWND hwnd, LPARAM state) -> BOOL { DWORD process; GetWindowThreadProcessId(hwnd, &process); if (process == GetCurrentProcessId() && GetPropW(hwnd, L"Frame.Window.main")) { reinterpret_cast<Search *>(state)->hwnd = hwnd; return FALSE; } return TRUE; }, reinterpret_cast<LPARAM>(&found));
...
REACT_MODULE(FrameNotifications, L"NativeDesktopNotifications")
struct FrameNotifications {
  ...
  ~FrameNotifications() { auto value = state; if (value->context) value->context.UIDispatcher().Post([value]() { value->Close(); }); }

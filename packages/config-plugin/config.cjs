...
const managedConfig = "// Generated by frame: universal Expo configuration";
...
function statePath(root, name, target = process.env.FRAME_PLATFORM) {
  if (!isUniversal(root)) return path.join(root, ".frame", name);
  const value = JSON.parse(fs.readFileSync(path.join(root, filename), "utf8"));
  target = selectTarget(value.platforms, target);
  return path.join(root, ".frame", "platforms", target, name);
}
...
function readConfig(root, target = process.env.FRAME_PLATFORM) {
  const file = path.join(root, filename);
  if (isExpoProject(root)) {
    ...
    try {
      process.env.FRAME_PLATFORM = selected;
      ...
      if (["macos", "windows"].includes(selected) && expo.extra?.frame?.projectId !== value.projectId) throw new Error("Compose app.config with withFrameExpo before running desktop commands");
...
function expoConfig(root) {
  if (process.env.FRAME_DEV_SESSION === "1") {
    return developmentConfig(JSON.parse(fs.readFileSync(path.join(root, filename), "utf8")), process.env.FRAME_PLATFORM);
  }
...
function toExpo(value, target) {
  ...
  const config = {
    ...
    extra: { ...backend.extra, frame: { ...extra, ...(platforms.length > 1 ? { supportedPlatforms: platforms } : {}) } },
    ...
    plugins: [...(["macos", "windows"].includes(selected) ? ["@legendapp/frame/config-plugin"] : []), ...(backend.plugins ?? []).filter(p => (Array.isArray(p) ? p[0] : p) !== "@legendapp/frame-desktop-config" && (Array.isArray(p) ? p[0] : p) !== "@legendapp/frame/config-plugin")],
  };
  identity(config);
  return { expo: config };
}
...
function prepareConfig(root) {
  ...
  if (isUniversal(root)) {
    ...
    if (fs.existsSync(path.join(root, "app.config.ts")) || (fs.existsSync(file) && !fs.readFileSync(file, "utf8").startsWith(managedConfig))) throw new Error("Universal configuration needs a frame-managed app.config.js; preserve custom configuration under expo or expoByPlatform");
    const result = readConfig(root);
...
function writeUpdates(root, updates) {
  ...
  if (fs.existsSync(file)) {
    ...
    const value = readConfig(root); value.expo.extra ??= {}; value.expo.extra.frame ??= {}; value.expo.extra.frame.updates = updates;

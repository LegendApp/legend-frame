...
function expoConfig(root) {
  ...
  if (isUniversal(root)) {
    const selection = statePath(root, "native-selection.json", result.expo.platforms[0]);
    if (result.expo.platforms[0] === "macos" && fs.existsSync(selection)) {
      const excluded = JSON.parse(fs.readFileSync(selection, "utf8")).excluded ?? [];
      result.expo.autolinking = { ...result.expo.autolinking, exclude: [...new Set([...(result.expo.autolinking?.exclude ?? []), ...excluded])] };
    }
  }
  return result.expo;
}
...
module.exports = { readConfig, toExpo, prepareConfig, writeUpdates, statePath, isUniversal, selectTarget, expoConfig };

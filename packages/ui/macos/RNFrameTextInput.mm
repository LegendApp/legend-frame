...
@implementation RNFrameTextInput {
  NSTextField *_field;
  BOOL _initialized;
}
- (instancetype)init {
  if (self = [super init]) {
    _props = std::make_shared<const FrameTextInputProps>();
...
- (void)controlTextDidChange:(NSNotification *)notification {
  const auto emitter = std::static_pointer_cast<const FrameTextInputEventEmitter>(_eventEmitter);
  if (emitter) emitter->onTextChange({ .text = _field.stringValue.UTF8String ?: "" });
}
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
  const auto &value = *std::static_pointer_cast<const FrameTextInputProps>(props);
...
+ (ComponentDescriptorProvider)componentDescriptorProvider { return concreteComponentDescriptorProvider<FrameTextInputComponentDescriptor>(); }
@end

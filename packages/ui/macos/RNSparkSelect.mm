...
@implementation RNSparkSelect {
  NSPopUpButton *_select;
  NSString *_items;
}
- (instancetype)init {
  if (self = [super init]) {
    _props = std::make_shared<const SparkSelectProps>();
...
- (void)changed:(id)sender {
  const auto emitter = std::static_pointer_cast<const SparkSelectEventEmitter>(_eventEmitter);
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
  const auto &value = *std::static_pointer_cast<const SparkSelectProps>(props);
...
+ (ComponentDescriptorProvider)componentDescriptorProvider { return concreteComponentDescriptorProvider<SparkSelectComponentDescriptor>(); }
@end

...
@implementation RNFrameSelect {
  NSPopUpButton *_select;
  NSString *_items;
}
- (instancetype)init {
  if (self = [super init]) {
    _props = std::make_shared<const FrameSelectProps>();
...
- (void)changed:(id)sender {
  const auto emitter = std::static_pointer_cast<const FrameSelectEventEmitter>(_eventEmitter);
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
  const auto &value = *std::static_pointer_cast<const FrameSelectProps>(props);
...
+ (ComponentDescriptorProvider)componentDescriptorProvider { return concreteComponentDescriptorProvider<FrameSelectComponentDescriptor>(); }
@end

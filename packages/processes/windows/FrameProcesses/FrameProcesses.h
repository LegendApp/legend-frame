...
namespace winrt::FrameProcesses {
  ...
  void Start(Json::JsonObject const &args) {
    ...
    if (executable.rfind(L"helper:", 0) == 0) {
      ...
      if (std::filesystem::exists(bundle)) {
        std::ifstream metadata(bundle / L".frame-entry", std::ios::binary);
...
REACT_MODULE(FrameProcesses, L"NativeDesktopProcesses")
struct FrameProcesses {
  ...
  ~FrameProcesses() { auto current = state; *current->active = false; if (current->context) current->context.UIDispatcher().Post([current]() { for (auto const &[id, process] : current->processes) process->Stop(); }); }

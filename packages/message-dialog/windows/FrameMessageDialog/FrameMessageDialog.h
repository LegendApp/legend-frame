...
namespace winrt::FrameMessageDialog {
...
inline HWND Parent(std::wstring const &id, bool explicitId) {
  ...
  if (!explicitId && active && GetPropW(active, L"Frame.ReactWindow")) return active;
  struct Search { std::wstring property; HWND found = nullptr; } search{L"Frame.Window." + (explicitId ? id : L"main")};
...
inline void Schedule(React::ReactContext context, std::function<void()> action, std::function<void()> failure) {
  context.UIDispatcher().Post([action = std::move(action), failure = std::move(failure)]() {
    const wchar_t *name = L"FrameMessageDialogTask";
...
struct DisabledWindows {
  ...
  void DisableOthers(HWND parent) {
    ...
    EnumWindows([](HWND window, LPARAM value) -> BOOL {
      ...
      if (process == GetCurrentProcessId() && window != input->parent && GetPropW(window, L"Frame.ReactWindow") && IsWindowEnabled(window)) {
...
REACT_MODULE(FrameMessageDialog, L"NativeDesktopMessageDialog")
struct FrameMessageDialog {

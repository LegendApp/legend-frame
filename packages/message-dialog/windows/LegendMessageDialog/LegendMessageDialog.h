...
namespace winrt::LegendMessageDialog {
...
inline HWND Parent(std::wstring const &id, bool explicitId) {
  ...
  if (!explicitId && active && GetPropW(active, L"Legend.ReactWindow")) return active;
  struct Search { std::wstring property; HWND found = nullptr; } search{L"Legend.Window." + (explicitId ? id : L"main")};
...
inline void Schedule(React::ReactContext context, std::function<void()> action, std::function<void()> failure) {
  context.UIDispatcher().Post([action = std::move(action), failure = std::move(failure)]() {
    const wchar_t *name = L"LegendMessageDialogTask";
...
struct DisabledWindows {
  ...
  void DisableOthers(HWND parent) {
    ...
    EnumWindows([](HWND window, LPARAM value) -> BOOL {
      ...
      if (process == GetCurrentProcessId() && window != input->parent && GetPropW(window, L"Legend.ReactWindow") && IsWindowEnabled(window)) {
...
REACT_MODULE(LegendMessageDialog, L"NativeDesktopMessageDialog")
struct LegendMessageDialog {

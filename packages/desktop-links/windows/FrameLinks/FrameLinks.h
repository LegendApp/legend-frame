...
namespace winrt::FrameLinks {
namespace React = Microsoft::ReactNative;
namespace Json = Windows::Data::Json;
inline std::wstring ProjectAppId() {
  DWORD size = GetEnvironmentVariableW(L"FRAME_PROJECT_ID", nullptr, 0);
  if (!size) throw hresult_invalid_argument(L"Missing project identity");
  std::wstring identity(size, L'\0'); identity.resize(GetEnvironmentVariableW(L"FRAME_PROJECT_ID", identity.data(), size));
  using namespace Windows::Security::Cryptography;
  auto hash = Core::HashAlgorithmProvider::OpenAlgorithm(Core::HashAlgorithmNames::Sha256());
  return L"Frame." + std::wstring(CryptographicBuffer::EncodeToHexString(hash.HashData(CryptographicBuffer::ConvertStringToBinary(identity, BinaryStringEncoding::Utf8))));
}
struct RecentDocuments {
  HKEY key = nullptr;
  std::wstring appId = ProjectAppId();
  RecentDocuments() { check_win32(RegCreateKeyExW(HKEY_CURRENT_USER, (L"Software\\Frame\\" + appId + L"\\RecentDocuments").c_str(), 0, nullptr, 0, KEY_READ | KEY_WRITE, nullptr, &key, nullptr)); }
  ...
  bool ShellEnabled() { wchar_t mode[16]{}; GetEnvironmentVariableW(L"FRAME_RUNTIME_MODE", mode, 16); return std::wstring(mode) == L"dev"; }
...
REACT_MODULE(FrameLinks, L"NativeDesktopLinks")
struct FrameLinks {
  using Receivers = std::map<std::string, std::unique_ptr<frame::AuthLoopback>>;
    ...
    try {
      ...
      if (method == "cryptoRandom") {
        ...
        if (method == "authPrepare") {
          ...
          auto receiver = std::make_unique<frame::AuthLoopback>(static_cast<unsigned short>(port), to_string(args.GetNamedString(L"path")));

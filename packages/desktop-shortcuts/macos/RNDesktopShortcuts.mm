...
- (void)call:(NSString *)method args:(NSString *)json resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if ([method isEqual:@"register"]) {
      ...
      if (!self.monitor) {
        ...
        self.monitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown handler:^NSEvent *(NSEvent *event) {
          ...
          NSString *windowID = [event.window.identifier hasPrefix:@"legend."] ? [event.window.identifier substringFromIndex:7] : nil;
          NSDictionary *match;
    ...
    } else { LegendInvalid(reject, @"Unknown shortcut operation"); return; }
    resolve(@"null");
  });
}
- (void)invalidate {

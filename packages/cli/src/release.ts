...
export const releaseBase = (version: string) => `https://github.com/LegendApp/legend-frame/releases/download/v${version}`;
export function validateRelease(value: any): ReleaseManifest {
  if (value?.schema !== 1 || value.version !== VERSION || !/^[a-f0-9]{40}$/.test(value.revision) || !value.packages || !value.runners) throw new Error("Invalid or incompatible Frame release manifest");
  for (const asset of [...Object.values(value.packages), ...Object.values(value.runners)] as ReleaseAsset[]) {
    const url = new URL(asset.url);
    if (url.href !== asset.url || !/^[A-Za-z0-9._-]+$/.test(url.pathname.split("/").at(-1) ?? "") || !asset.url.startsWith(releaseBase(VERSION) + "/") || url.search || url.hash || !/^[a-f0-9]{64}$/.test(asset.sha256) || !Number.isSafeInteger(asset.size) || asset.size <= 0) throw new Error("Invalid Frame release asset");
  }
...
export function packageSources(manifest?: string, release = installedRelease()): Record<string, string> {
  ...
  if (manifest) {
    ...
    sources = { ...Object.fromEntries(Object.entries(release.packages).map(([name, asset]) => [name, asset.url])), "@legendapp/frame": VERSION };
  }
  return sources;
}

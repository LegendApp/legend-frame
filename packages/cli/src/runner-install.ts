...
import { frameHome, readRuntime, registerRuntime } from "./local.ts";
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      for await (const chunk of response.body as any as AsyncIterable<Uint8Array>) {
        ...
        if (percent !== reported) { console.log(`Downloading Frame Runner: ${percent}%`); reported = percent; }
      }
...
export async function installRunner(asset: RunnerAsset, cache = frameHome(), deps: Dependencies = defaults) {
  ...
  try {
    ...
    registerRuntime(app);
    console.log("Frame Runner installed and verified.");
    return app;
  } finally {
...
export async function acquireRunner(platform: DesktopPlatform) {
  ...
  if (release) {
    ...
    if (!asset) throw new Error(`This Frame release has no Runner for ${target}. Use a custom development build.`);

...
export async function credentials(root: string, reset = false, execute: Runner = run): Promise<SigningCredentials> {
  ...
  const identity = process.env.SPARK_DEVELOPER_ID_APPLICATION ?? config.identity ?? saved.hash;
  const teamId = process.env.SPARK_TEAM_ID ?? config.teamId ?? saved.teamId;
  ...
  if (matches.length > 1) {
    if (!process.stdin.isTTY) throw new Error("Multiple signing identities are available. Set SPARK_DEVELOPER_ID_APPLICATION or run spark credentials interactively.");

...
export function chooseIdentity(identities: SigningIdentity[], identity?: string, teamId?: string) {
  ...
  if (!matches.length) throw new Error("No matching Developer ID Application identity with a private key is available. Install it through Xcode or Keychain Access, then run frame credentials.");
  return matches;
}
...
export async function credentials(root: string, reset = false, execute: Runner = run): Promise<SigningCredentials> {
  ...
  const config = readAppConfig(root).expo?.extra?.frame?.signing?.macos ?? {};
  const keychain = process.env.FRAME_SIGNING_KEYCHAIN ?? saved.keychain;
  const identity = process.env.FRAME_DEVELOPER_ID_APPLICATION ?? config.identity ?? saved.hash;
  const teamId = process.env.FRAME_TEAM_ID ?? config.teamId ?? saved.teamId;
  ...
  if (matches.length > 1) {
    if (!process.stdin.isTTY) throw new Error("Multiple signing identities are available. Set FRAME_DEVELOPER_ID_APPLICATION or run frame credentials interactively.");
  ...
  let keychainProfile = process.env.FRAME_NOTARY_KEYCHAIN_PROFILE ?? saved.keychainProfile;
  if (!keychainProfile) {
    if (!process.stdin.isTTY) throw new Error("Notarization is not configured. Set FRAME_NOTARY_KEYCHAIN_PROFILE or run frame credentials interactively.");
    keychainProfile = await ask("Existing notarization Keychain profile (Enter to create one): ");
    if (!keychainProfile) {
      keychainProfile = `frame-${selected.teamId}`;
      console.log(`Apple’s tool will store and validate notarization credentials in Keychain as ${keychainProfile}. Secret input is handled by notarytool and is not recorded by frame.`);
      const child = Bun.spawn(["xcrun", "notarytool", "store-credentials", keychainProfile, ...(keychain ? ["--keychain", keychain] : [])], {
        stdin: "inherit", stdout: "inherit", stderr: "inherit",
      });
      if (await child.exited) throw new Error("Notarization credential setup failed. Run frame credentials to retry.");
    }
  }

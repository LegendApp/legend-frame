...
const { resolveEntitlements } = createRequire(import.meta.url)("@legend-apps/desktop-config/entitlements.cjs");
...
export async function validateApp(root: string, app: string, credentials: SigningCredentials, expected: { bundleId: string; version: string; buildVersion: string; entitlements: Entitlements; byPath: Record<string, Entitlements> }, notarized: boolean, execute: Runner = run) {
  app = realpathSync(app);
  if (!existsSync(path.join(app, "Contents/Resources/main.jsbundle"))) throw new Error("Distribution app is missing its JavaScript bundle.");

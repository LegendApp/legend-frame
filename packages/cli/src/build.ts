...
async function buildUnlocked(
  ...
  if (
    ...
    !(isExpoProject(root) && readFileSync(rnConfig, "utf8").includes("// Legend: existing Expo project"))
  )
    throw new Error(
  ...
  await run(
    ...
    {
      env: { RCT_NEW_ARCH_ENABLED: "1", ENTRY_FILE: entryFile(root), ...(productionGraph ? { LEGEND_RUNTIME_SOURCES: stateFile(root, "analysis/runtime-sources.json") } : {}) },
      capture: true,
    },
  );

...
export function installedPackages(root: string): Package[] {
  ...
  while (queue.length) {
    ...
    const req = createRequire(path.join(current.root, current.json.frame?.bundledModuleRoot ?? ".", "package.json"));
    const bundled = current.json.frame?.bundledModules ?? current.json.bundledDependencies ?? current.json.bundleDependencies;
...
export function goConfigurationIssues(config: any): string[] {
  ...
  if (
    (expo.plugins ?? []).some(
      (plugin: any) =>
        !["@legendapp/frame-desktop-config", "@legendapp/frame/config-plugin"].includes(Array.isArray(plugin) ? plugin[0] : plugin),
    )
  )
...
export function validateBuildModules(mode: string, packages: NativePackage[]) {
  if (mode === "go" && packages.some(pkg => pkg.name === "@legendapp/frame-native-greeting" || pkg.json.frame?.testOnly))
    throw new Error("Build the Frame Runner from the clean SDK starter, not a custom-module test fixture.");
...
export function projectEnvironment(root: string): Record<string, string> {
  ...
  if (typeof projectId !== "string" || !projectId.length || projectId.length > 200)
    throw new Error("Set extra.frame.projectId to a stable project identifier before launching the Frame Runner.");
  return {

...
export function composeMetro(source: string, file: string) {
  ...
  for (const { start, end } of replacements.sort((a, b) => b.start - a.start)) source = source.slice(0, start) + '"@legendapp/frame/expo-metro"' + source.slice(end);
...
export async function addDesktop(root: string, manifestFile: string) {
  ...
  files.set(configFile, composeExport(configSource, configFile, "@legendapp/frame/expo-config", "withFrameExpo"));

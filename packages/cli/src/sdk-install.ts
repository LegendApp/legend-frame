...
if (!overrides["@legend-apps/cli"]) throw new Error("SDK has no CLI");
const tooling = path.join(root, ".cli");
mkdirSync(tooling, { recursive: true });
writeFileSync(path.join(tooling, "package.json"), JSON.stringify({ name: "legend-sdk-tools", private: true, dependencies: { "@legend-apps/cli": overrides["@legend-apps/cli"] }, overrides }, null, 2));
...
const cli = path.join(tooling, "node_modules/@legend-apps/cli/src/index.ts");

...
try {
  ...
  foreach ($scheme in $plan.protocols) {
    ...
    if ($key) {
      try { if ($key.GetValue('LegendOwner') -ne $plan.appId) { throw "Protocol '$scheme' belongs to another app" } } finally { $key.Dispose() }
    }
  }
  ...
  if ($Remove) {
    ...
    try {
      $key.SetValue('', [string]$plan.name); $key.SetValue('LegendOwner', [string]$plan.appId)
    ...
    foreach ($scheme in $plan.protocols) {
      ...
      try {
        $key.SetValue('', 'URL:' + $plan.name); $key.SetValue('URL Protocol', ''); $key.SetValue('LegendOwner', [string]$plan.appId)
        $open = $key.CreateSubKey('shell\open\command'); try { $open.SetValue('', $command) } finally { $open.Dispose() }
      } finally { $key.Dispose() }
    }
  }
  ...
  if ($plan.previous) {
    foreach ($scheme in $plan.previous.protocols) {
      ...
      $key = $classes.OpenSubKey($scheme)
      $owned = $false
      if ($key) { try { $owned = $key.GetValue('LegendOwner') -eq $plan.previous.appId } finally { $key.Dispose() } }
  ...
  Add-Type -TypeDefinition '[System.Runtime.InteropServices.DllImport("shell32.dll")] public static extern void SHChangeNotify(uint e, uint f, System.IntPtr a, System.IntPtr b);' -Name Shell -Namespace Legend
  [Legend.Shell]::SHChangeNotify(0x08000000, 0, [IntPtr]::Zero, [IntPtr]::Zero)

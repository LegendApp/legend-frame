...
export function verifySDK(root: string) {
  ...
  for (const relative of metadata.runtimes) {
    if (typeof relative !== "string" || !relative.startsWith("runtimes/") || relative.includes("..") || !readRuntime(path.join(root, relative))) throw new Error("Invalid bundled Frame Runner");
  }
  return metadata;
}
...
export function exportSDK(manifest: string, destination: string, runtimes: string[] = []) {
  ...
  try {
    ...
    for (const app of runtimes) {
      ...
      if (!runtime || runtime.mode !== "go") throw new Error(`Not a compatible Frame Runner: ${app}`);
      const relative = `runtimes/${runtime.platform}-${runtime.arch}/${path.basename(app)}`;
      if (clients.includes(relative)) throw new Error("Duplicate Frame Runner target");
...
Keep this directory in place after installing. Its package archives and optional Frame Runner runtimes are registered by path. Transfer before installing, preserving executable permissions and symlinks. Third-party npm dependencies still require internet access.

Included runtimes: ${clients.length ? clients.join(", ") : "none (install a matching Frame Runner or build a development client)"}. Compatibility checks reject mismatched native modules. Native Windows compilation and clean-machine acceptance require Windows; public hosting and production signing are separate workflows.
`);
    verifySDK(pending);

...
export async function prepareGoProfile(root: string, manifest: string, platform: DesktopPlatform) {
  ...
  if (!archives["@legendapp/frame"]) throw new Error("Frame Runner profile requires @legendapp/frame");
  pkg.dependencies["@legendapp/frame"] = localArchive(path.resolve(path.dirname(manifest), archives["@legendapp/frame"]));

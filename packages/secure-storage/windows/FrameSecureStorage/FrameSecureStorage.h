...
namespace winrt::FrameSecureStorage {
namespace React = Microsoft::ReactNative;
namespace Json = Windows::Data::Json;
REACT_MODULE(FrameSecureStorage, L"NativeDesktopSecureStorage")
struct FrameSecureStorage {
  REACT_METHOD(call)
  void call(std::string method, std::string encoded, React::ReactPromise<std::string> promise) noexcept {
    try {
      wchar_t project[201]{};
      const auto count = GetEnvironmentVariableW(L"FRAME_PROJECT_ID", project, 201);
      if (!count || count >= 201) { promise.Reject(React::ReactError{"E_IDENTITY", "Launch through Frame to establish project-scoped storage"}); return; }
      ...
      std::wstring target = std::wstring(L"Frame/") + std::to_wstring(count) + L":" + project + L"/" + key.c_str();
      if (method == "get") {

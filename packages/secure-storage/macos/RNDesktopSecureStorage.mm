...
- (void)call:(NSString *)method args:(NSString *)json resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
    ...
    if (![key isKindOfClass:NSString.class] || !key.length || key.length > 200) { LegendInvalid(reject, @"Expected a Keychain key of 1–200 characters"); return; }
    // Callers cannot override service identity and accidentally collide in Go.
    NSMutableDictionary *query = [@{ (__bridge id)kSecClass: (__bridge id)kSecClassGenericPassword,
      (__bridge id)kSecAttrService: LegendNamespace(), (__bridge id)kSecAttrAccount: key } mutableCopy];
    OSStatus status;
    if ([method isEqual:@"get"]) {
      ...
      if (![args[@"value"] isKindOfClass:NSString.class]) { LegendInvalid(reject, @"Expected a string value"); return; }
    ...
    } else { LegendInvalid(reject, @"Unknown Keychain operation"); return; }

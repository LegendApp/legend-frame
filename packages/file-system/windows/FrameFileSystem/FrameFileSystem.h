...
inline void Write(fs::path const &path, std::vector<uint8_t> const &data) {
  ...
  auto temporary = path.parent_path() / (std::wstring(L".frame-") + id);
  try {
...
inline fs::path Directory(hstring const &kind) {
  wchar_t identity[201]{}; auto count = GetEnvironmentVariableW(L"FRAME_PROJECT_ID", identity, 201);
  if (!count || count >= 201) throw hresult_invalid_argument(L"Launch through Frame to establish project storage identity");
  ...
  base /= L"Frame"; for (size_t i = 0; i < project.size(); i += 100) base /= project.substr(i, 100);
...
REACT_MODULE(FrameFileSystem, L"NativeDesktopFileSystem")
struct FrameFileSystem {

...
inline void Write(fs::path const &path, std::vector<uint8_t> const &data) {
  ...
  auto temporary = path.parent_path() / (std::wstring(L".legend-") + id);
  try {
...
inline fs::path Directory(hstring const &kind) {
  wchar_t identity[201]{}; auto count = GetEnvironmentVariableW(L"LEGEND_PROJECT_ID", identity, 201);
  if (!count || count >= 201) throw hresult_invalid_argument(L"Launch through Legend to establish project storage identity");
  ...
  base /= L"Legend"; for (size_t i = 0; i < project.size(); i += 100) base /= project.substr(i, 100);
...
REACT_MODULE(LegendFileSystem, L"NativeDesktopFileSystem")
struct LegendFileSystem {

...
@interface LegendOpenFile : NSObject
@property int descriptor;
@end
@implementation LegendOpenFile
...
@property NSMutableDictionary<NSString *, LegendOpenFile *> *files;
...
- (instancetype)init { if (self = [super init]) { _ioQueue = dispatch_queue_create("legend.files", DISPATCH_QUEUE_SERIAL); _files = [NSMutableDictionary new]; _watches = [NSMutableDictionary new]; _recursiveWatches = [NSMutableDictionary new]; } return self; }
...
- (void)call:(NSString *)method args:(NSString *)json resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  dispatch_async(self.ioQueue, ^{
    ...
    if ([@[@"readChunk", @"writeChunk", @"flushFile", @"closeFile"] containsObject:method]) {
      NSString *identifier = args[@"id"]; LegendOpenFile *file = self.files[identifier];
      ...
      if ([method isEqual:@"flushFile"]) { if (fsync(file.descriptor)) error = [NSError errorWithDomain:NSPOSIXErrorDomain code:errno userInfo:nil]; }
        ...
        if (![args[@"offset"] isKindOfClass:NSNumber.class] || !isfinite(position) || position < 0 || floor(position) != position || position > 9007199254740991.0) { LegendInvalid(reject, @"Invalid file offset"); return; }
        if ([method isEqual:@"readChunk"]) {
          double size = [args[@"length"] doubleValue];
          if (!isfinite(size) || size < 1 || size > 1048576 || floor(size) != size || position + size > 9007199254740991.0) { LegendInvalid(reject, @"Invalid chunk length"); return; }
          ...
          if (![base64 isKindOfClass:NSString.class] || base64.length > 1398104) { LegendInvalid(reject, @"Chunk exceeds 1 MiB"); return; }
          NSData *data = [[NSData alloc] initWithBase64EncodedString:base64 options:0];
          if (!data || data.length > 1048576 || position + data.length > 9007199254740991.0) { LegendInvalid(reject, @"Invalid chunk"); return; }
      ...
      if ([method isEqual:@"openFile"]) {
        ...
        else { LegendInvalid(reject, @"Invalid file mode"); return; }
        ...
        if (fd < 0) error = [NSError errorWithDomain:NSPOSIXErrorDomain code:errno userInfo:nil];
          ...
          else { LegendOpenFile *file = [LegendOpenFile new]; file.descriptor = fd; NSString *identifier = NSUUID.UUID.UUIDString; self.files[identifier] = file; result = identifier; }
        }
...
- (void)invalidate {
  dispatch_async(self.ioQueue, ^{ [self.files removeAllObjects]; for (LegendRecursiveWatch *watch in self.recursiveWatches.allValues) [watch stop]; [self.recursiveWatches removeAllObjects]; for (dispatch_source_t source in self.watches.allValues) dispatch_source_cancel(source); [self.watches removeAllObjects]; });
  [super invalidate];
}

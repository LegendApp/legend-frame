...
NSDictionary *FrameWindowConfiguration(void) {
#if DEBUG
  if ([FrameContext()[@"runtime"][@"mode"] isEqual:@"go"]) {
    NSString *json = NSProcessInfo.processInfo.environment[@"FRAME_WINDOW_CONFIG"];
    return json ? FrameArgs(json) : @{};
  }
#endif
  return [NSBundle.mainBundle objectForInfoDictionaryKey:@"FrameWindowConfiguration"] ?: @{};
}
NSView *FrameWindowContent(NSView *root) {
...
void FrameApplyWindowOptions(NSWindow *window, NSDictionary *patch) {
...
void FrameRestoreWindow(NSWindow *window, NSString *key, NSDictionary *options) {
  [window center];
  if ([options[@"restoreFrame"] boolValue]) {
    NSString *name = [NSString stringWithFormat:@"%@.%@", FrameNamespace(), key];
    ...
    FrameApplyWindowOptions(window, @{});

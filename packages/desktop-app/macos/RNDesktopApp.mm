...
extern void LegendSetQuitGuard(BOOL value);
extern void LegendReplyQuit(BOOL allow, NSUInteger generation);
...
- (void)startObserving { [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(event:) name:LegendDesktopEvent object:nil]; }
...
- (void)invalidate { [self stopObserving]; dispatch_async(dispatch_get_main_queue(), ^{ LegendSetQuitGuard(NO); }); [super invalidate]; }
- (void)call:(NSString *)method args:(NSString *)json resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    else if ([method isEqual:@"quitGuard"]) { LegendSetQuitGuard([args[@"enabled"] boolValue]); resolve(@"null"); }
    else if ([method isEqual:@"replyQuit"]) { LegendReplyQuit([args[@"allow"] boolValue], [args[@"requestId"] unsignedIntegerValue]); resolve(@"null"); }
    ...
    else LegendInvalid(reject, @"Unknown app operation");
  });
}

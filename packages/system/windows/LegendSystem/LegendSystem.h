...
inline HWND MainWindow() {
  ...
  EnumWindows([](HWND hwnd, LPARAM value) -> BOOL { DWORD process; GetWindowThreadProcessId(hwnd, &process); if (process == GetCurrentProcessId() && GetPropW(hwnd, L"Legend.Window.main")) { reinterpret_cast<Search *>(value)->result = hwnd; return FALSE; } return TRUE; }, reinterpret_cast<LPARAM>(&search));
...
inline std::wstring AppId() {
  auto project = Env(L"LEGEND_PROJECT_ID"); if (project.empty()) throw hresult_invalid_argument(L"Missing project identity");
  using namespace Windows::Security::Cryptography;
  auto hash = Core::HashAlgorithmProvider::OpenAlgorithm(Core::HashAlgorithmNames::Sha256());
  return L"Legend." + std::wstring(CryptographicBuffer::EncodeToHexString(hash.HashData(CryptographicBuffer::ConvertStringToBinary(project, BinaryStringEncoding::Utf8))));
}
struct SystemState {
  ...
  void EnsureWindow() {
    if (hwnd) return;
    WNDCLASSW cls{}; cls.lpfnWndProc = Proc; cls.lpszClassName = L"LegendSystem"; cls.hInstance = GetModuleHandleW(nullptr);
  ...
  void DockMenu(Json::JsonObject const &args) {
    ...
    try {
      ...
      for (auto const &value : args.GetNamedArray(L"items")) {
        ...
        for (auto key : {L"LEGEND_PROJECT_ID", L"LEGEND_PROJECT_NAME", L"LEGEND_PROJECT_VERSION", L"LEGEND_WINDOW_CONFIG", L"LEGEND_SESSION_FILE"}) launch.SetNamedValue(key, Json::JsonValue::CreateStringValue(Env(key)));
        auto command = L"--legend-context=" + std::wstring(Windows::Foundation::Uri::EscapeComponent(launch.Stringify())) + L" --legend-dock-action=" + std::wstring(payload); check_hresult(link->SetArguments(command.c_str()));
...
REACT_MODULE(LegendSystem, L"NativeDesktopSystem")
struct LegendSystem {
  ...
  ~LegendSystem() { auto current = state; if (current->context) current->context.UIDispatcher().Post([current]() { current->Close(); }); }

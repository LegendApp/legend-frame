...
struct TrayState : std::enable_shared_from_this<TrayState> {
  ...
  NOTIFYICONDATAW Data(UINT key, Item const &item) {
    ...
    auto tip = item.options.GetNamedString(L"tooltip", item.options.GetNamedString(L"title", L"Frame"));
  ...
  void EnsureWindow() {
    if (hwnd) return;
    WNDCLASSW cls{}; cls.lpfnWndProc = Proc; cls.lpszClassName = L"FrameTray"; cls.hInstance = GetModuleHandleW(nullptr);
...
REACT_MODULE(FrameTray, L"NativeDesktopTray")
struct FrameTray {
  ...
  ~FrameTray() { auto current = std::move(state); if (current->context) current->context.UIDispatcher().Post([current]() { current->Close(); }); }
  REACT_METHOD(call)

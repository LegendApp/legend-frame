...
@interface LegendTrayItem : NSObject
...
@end
@implementation LegendTrayItem
- (void)clicked:(id)sender { LegendEmit(@{ @"type": @"trayClick", @"trayId": self.identifier }); }
- (void)selected:(NSMenuItem *)sender { LegendEmit(@{ @"type": @"trayAction", @"trayId": self.identifier, @"itemId": sender.representedObject }); }
@end
static NSMenu *Menu(NSArray *items, LegendTrayItem *owner) {
...
@property NSMutableDictionary<NSString *, LegendTrayItem *> *items;
@end
...
- (void)call:(NSString *)method args:(NSString *)json resolve:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (![key isKindOfClass:NSString.class] || !key.length) { LegendInvalid(reject, @"Tray needs an id"); return; }
    LegendTrayItem *owner = self.items[key];
    ...
    if (![method isEqual:@"create"] && ![method isEqual:@"update"]) { LegendInvalid(reject, @"Unknown tray operation"); return; }
    ...
    if ([options[@"symbol"] length]) {
      ...
      if (!image) { LegendInvalid(reject, @"Unknown SF Symbol name"); return; }
      [image setTemplate:YES];
    }
    if (![options[@"title"] length] && !image) { LegendInvalid(reject, @"Tray needs a title or symbol"); return; }
    if (!owner) { owner = [LegendTrayItem new]; owner.identifier = key; owner.item = [NSStatusBar.systemStatusBar statusItemWithLength:NSVariableStatusItemLength]; self.items[key] = owner; }
    ...
    owner.item.button.accessibilityIdentifier = [@"legend.tray." stringByAppendingString:key];
    owner.item.menu = [options[@"menu"] count] ? Menu(options[@"menu"], owner) : nil;
    resolve(@"null");
  });
}
- (void)invalidate { dispatch_async(dispatch_get_main_queue(), ^{ for (LegendTrayItem *owner in self.items.allValues) [NSStatusBar.systemStatusBar removeStatusItem:owner.item]; [self.items removeAllObjects]; }); }

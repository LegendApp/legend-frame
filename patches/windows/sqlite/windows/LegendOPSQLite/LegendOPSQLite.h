...
REACT_MODULE(LegendOPSQLiteModule, L"OPSQLite")
struct LegendOPSQLiteModule {
  ...
  ~LegendOPSQLiteModule() { if (generation) opsqlite::invalidate(generation); }
  static std::filesystem::path Directory() {
    wchar_t project[32768]{}; if (!GetEnvironmentVariableW(L"LEGEND_PROJECT_ID", project, 32768)) throw winrt::hresult_invalid_argument(L"Missing project identity");
    ...
    path /= L"Legend"; path /= id.c_str(); path /= L"data"; std::filesystem::create_directories(path); return path;
  }

...
- Use a public distribution repository. The currently configured repository is
  `LegendApp/legend-frame`; its visibility must be made public explicitly before
...
- Install a Developer ID Application certificate and private key. Set
  `FRAME_DEVELOPER_ID_APPLICATION` and `FRAME_NOTARY_KEYCHAIN_PROFILE` to the
  chosen identity/profile references (or configure them with `frame credentials`).
...
`frame sdk package-runner --submission-id <Apple submission UUID>`; do not retry
an uncertain submission by discarding its state. Existing signing/notarization
state remains in the managed project's `.frame/packaging` directory.
...
```sh
npx @legendapp/frame@next create MyApp
cd MyApp
npm run dev
```

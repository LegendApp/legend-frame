...
and the pinned Expo Desktop beta. Source is in `legend-framework`; builds ran
from `/tmp/legend-sdk-validation` because Bun stalls in the Documents checkout.
...
| Saved Go artifact | Pass: copied to `artifacts/runtimes/LegendGo.app`, `codesign --verify --deep --strict` succeeds, registered with the local SDK |
...
The full regression used `LEGEND_TEST_UI_DRIVER=external`; computer use clicked
...
bun run typecheck
bun test tests
bun run test:expansion /tmp/LegendSDKKitchenSink
bun run test:native /tmp/LegendSDKKitchenSink
bun run test:updates /tmp/LegendUpdateReleaseProbe
...
`LEGEND_TEST_UI_DRIVER=external` only when another UI driver or a person will

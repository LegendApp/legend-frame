...
| Saved Go artifact | Pass: copied to `artifacts/runtimes/FrameRunner.app`, `codesign --verify --deep --strict` succeeds, registered with the local SDK |
...
npm run typecheck
npm test
npm run test:expansion -- /tmp/FrameSDKKitchenSink
npm run test:native -- /tmp/FrameSDKKitchenSink
npm run test:updates -- /tmp/FrameUpdateReleaseProbe
...
`artifacts/packages/`; the refreshed Go app is in `artifacts/runtimes/FrameRunner.app`.

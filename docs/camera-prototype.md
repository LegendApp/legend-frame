...
npm run test:camera -- /tmp/FrameNitroProbe --probe-only

# Prepare, build, launch, and run the complete camera-free proof.
npm run test:camera -- /tmp/FrameCameraKitchenSink

# Build a standalone Release app and run the same proof without Metro.
npm run test:camera -- /tmp/FrameCameraKitchenSink --release

# Open an already-built app for manual testing.
npm run test:camera -- /tmp/FrameCameraKitchenSink --run-only --interactive
```

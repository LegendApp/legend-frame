...
On Windows, the actions target Windows. They are disabled when the target cannot run on the host. `g` switches between the prebuilt runtime and a custom development build; it never compiles automatically. `b` becomes available when a custom build is required. Switching binaries can reset React state. Selection is remembered in `.frame/settings.json`.
...
To prepare a signed, notarized distribution archive, run `bun run package` in a new starter, or `bunx --no-install frame package` in an existing app. First use discovers signing identities and configures a notarization Keychain profile. See [packaging](packaging.md) for setup, CI, and retry behavior.
...
```sh
bun run frame sdk register /path/to/FramePrebuilt.app
```

Packing also discovers the saved prototype binary at `artifacts/runtimes/FramePrebuilt.app`, if present. Registration stores local paths under `~/.frame/`; it does not duplicate the binaries. Keep the registered binaries in place. Set `FRAME_HOME` to isolate local registry state for testing.
...
The persisted runtime mode (`"go"`), registry entries, saved settings, and existing `FramePrebuilt`/`products/go` paths remain unchanged so registered binaries keep working. This terminology change itself does not require a native rebuild. Older validation reports retain the original name. Bare `build` still means a standalone release; `--preview` is unchanged.
...
```sh
bun add /absolute/path/to/frame/artifacts/packages/legendapp-frame-native-greeting-0.1.0-prototype.0.tgz
```
...
If the current custom binary is stale, press `b` to rebuild it, or run `frame build --dev` followed by `bun dev`. `frame build --dev --force` forces native regeneration and compilation for a custom development build. Generated native directories are disposable: author native changes in packages/config plugins.

## Inspect and build production selection

```sh
bunx --no-install frame analyze
bun run build
```

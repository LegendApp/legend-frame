...
Implemented in `spark`: the five agreed SDK groups, separate native
...
| Saved Go artifact | Refreshed `artifacts/runtimes/SparkPrebuilt.app`; strict deep code-signature verification passed |
...
Validation ran from a synchronized `/tmp/spark-sdk-validation` copy because Bun
file access stalls in this Mac’s Documents checkout. Native consumers were built
under `/tmp/SparkSDKKitchenSink`. Source and the lockfile are in the repository;

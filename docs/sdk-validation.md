...
Implemented in `legend-framework`: the five agreed SDK groups, separate native
...
| Saved Go artifact | Refreshed `artifacts/runtimes/LegendGo.app`; strict deep code-signature verification passed |
...
Validation ran from a synchronized `/tmp/legend-sdk-validation` copy because Bun
file access stalls in this Mac’s Documents checkout. Native consumers were built
under `/tmp/LegendSDKKitchenSink`. Source and the lockfile are in the repository;

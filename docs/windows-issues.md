...
| WIN-05 | Integrated prebuilt/custom-build host has passed generation and bundle checks; Windows native acceptance is pending. | `bun run test:windows --project C:\dev\LegendWindowsVerification` passes compilation, Hermes startup, Fast Refresh, native dependency incompatibility detection, and custom-build switching. |

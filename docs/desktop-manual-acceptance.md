...
and dead keys. Its report is `.legend/keyboard-tests/report.json`. This does not
...
```powershell
bun run test:windows:features --project C:\dev\LegendAcceptance
```
...
Collect `.legend/test-results/<run-id>.json` from each machine, plus any referenced

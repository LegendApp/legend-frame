...
`@legend-apps/desktop/<feature>`. These additions do not embed Node.
...
`include`, `signing`, and `helpers`. These replace `expo.extra.legend.*` nesting.
...
```ts
import { openWindow, setWindowOptions } from "@legend-apps/desktop/windows";
...
```ts
import { showMessage, confirm } from "@legend-apps/desktop/dialogs";

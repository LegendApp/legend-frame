...
bunx --no-install spark build --dev --platform ios --device "<simulator UDID>"
bun run ios

# Android: build/install on the selected emulator/device, then develop
bunx --no-install spark build --dev --platform android --device "<device name or ID>"
bun run android
```
...
For direct Expo commands, set the target explicitly, for example `SPARK_PLATFORM=ios bunx expo config` on macOS/Linux or `$env:SPARK_PLATFORM="ios"` before the command in PowerShell. An omitted target defaults to the host desktop if supported, then the first supported platform.

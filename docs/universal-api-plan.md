...
- Framework APIs can delegate to Expo/community implementations on mobile/web and Legend implementations on desktop.
...
| --- | --- |
| `@legend-apps/api/<feature>` | Expo-aligned shared capability contracts and platform-selected adapters |
| `@legend-apps/ui` | Shared controls backed by Expo UI where suitable, with desktop adapters and explicit extensions |
| `@legend-apps/ui/router` | Expo Router integration exporting `Windows`; plain control imports must not import Router |
| Existing native feature packages | Internal desktop implementations reused behind the new contracts |
| Existing `@legend-apps/desktop/*` | Compatibility entry points while internal consumers migrate |
...
```tsx
import { Windows } from '@legend-apps/ui/router';

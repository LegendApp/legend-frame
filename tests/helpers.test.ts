...
function fixture(run: (root: string) => void) {
  const root = mkdtempSync(path.join(os.tmpdir(), "legend-helpers-"));
...
test("helper selection uses target architecture and fingerprints all assets", () => fixture(root => {
  ...
  expect(readFileSync(path.join(output, ".legend-entry"), "utf8")).toBe("bin/backend");
...
test("configuration accepts target bundles and catches invalid declarations early", () => {
  ...
  expect(toExpo({ ...base, helpers }).expo.extra.legend.helpers).toEqual(helpers);
...
test("changing bundled assets invalidates a custom runtime fingerprint", () => fixture(root => {
  ...
  expect(readFileSync(path.join(root, "Win/Helpers/backend.helper/.legend-entry"), "utf8")).toBe("bin/backend");
}));

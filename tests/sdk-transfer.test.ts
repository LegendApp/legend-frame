...
test("SDK can move before registration; altered packages fail verification", () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "legend-sdk-transfer-")); const home = process.env.LEGEND_HOME;
  try {
    process.env.LEGEND_HOME = path.join(root, "home"); mkdirSync(path.join(root, "source"));
    const packages = Object.fromEntries(["cli", "desktop", "desktop-config"].map(name => [`@legend-apps/${name}`, `${name}.tgz`]));
  ...
  } finally { if (home === undefined) delete process.env.LEGEND_HOME; else process.env.LEGEND_HOME = home; rmSync(root, { recursive: true, force: true }); }
});
test("SDK refuses native-runtime symlinks outside its directory", () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "legend-sdk-link-"));

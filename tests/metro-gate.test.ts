...
test("desktop compatibility is isolated by request platform in one Metro session", () => {
  const root = mkdtempSync(path.join(os.tmpdir(), "spark-gate-universal-"));
  try {
    writeFileSync(path.join(root, "desktop.config.json"), JSON.stringify({ platforms: ["ios", "android", "web", "macos", "windows"] }));
    for (const platform of ["macos", "windows"]) {
      mkdirSync(path.join(root, ".spark/platforms", platform), { recursive: true });
      writeFileSync(path.join(root, ".spark/platforms", platform, "session.json"), JSON.stringify({ compatible: platform === "windows" }));
    }
    const served: string[] = [];
    ...
    writeFileSync(path.join(root, ".spark/platforms/macos/session.json"), "{");
    const response = { statusCode: 200, end(_body: string) {} };

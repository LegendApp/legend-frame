...
test("workspace apps resolve hoisted tools and detect host source edits", () => {
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), "legend-workspace-tools-")));
  const app = path.join(root, "examples/app"), host = path.join(root, "node_modules/@legend-apps/desktop-host"), tool = path.join(root, "node_modules/expo");
  for (const dir of [app, host, tool]) mkdirSync(dir, { recursive: true });
  try {
    writeFileSync(path.join(app, "package.json"), JSON.stringify({ dependencies: { "@legend-apps/desktop-host": "1", expo: "1" } }));
    writeFileSync(path.join(host, "package.json"), JSON.stringify({ name: "@legend-apps/desktop-host", version: "1" }));

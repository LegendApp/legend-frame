...
test('mobile React Native codegen excludes desktop providers alongside Expo autolinking', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'legend-codegen-'));
  try {
    writeFileSync(path.join(root, 'app.json'), JSON.stringify({ expo: { platforms: ['ios'], autolinking: { exclude: ['@legend-apps/message-dialog'] } } }));
    const config = require('../packages/cli/src/universal.cjs').nativeConfig(root);
    expect(config.dependencies['@legend-apps/message-dialog'].platforms.ios).toBeNull();
    expect(config.dependencies['@legend-apps/message-dialog'].platforms.android).toBeNull();

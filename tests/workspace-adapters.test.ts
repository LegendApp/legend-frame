...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "legend-workspace-adapter-"));
  const pkg = path.join(root, "node_modules/probe"); mkdirSync(pkg, { recursive: true });
  writeFileSync(path.join(root, "package.json"), JSON.stringify({ dependencies: { probe: "1.0.0" }, legendWorkspacePatches: { "probe@1.0.0": "probe.patch" } }));

...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "frame-manager-"));
...
test("override conversion keeps pins and writes each manager's supported field", () => {
  for (const manager of ["npm", "pnpm", "yarn", "bun"] as const) {
    const pkg: any = { overrides: { react: "19.1.4" }, dependencies: { "@legendapp/frame": "file:/tmp/frame.tgz" } };
    applyOverrides(pkg, { "@legendapp/frame": "file:/tmp/frame.tgz" }, manager);
    const pins = manager === "pnpm" ? pkg.pnpm.overrides : manager === "yarn" ? pkg.resolutions : pkg.overrides;
    expect(pins).toEqual({ react: "19.1.4", "@legendapp/frame": "file:/tmp/frame.tgz" });
    expect(pkg.dependencies["@legendapp/frame"]).toBe("file:/tmp/frame.tgz");
  }
});

...
export async function packRuntimes(root: string, output: string) {
  const cache = path.join(root, ".frame/vendor/runtimes");
  ...
  // Native backend is compiled into frame's host; retain the upstream API.
  ...
  writeFileSync(path.join(core, "windows/README.md"), "Windows native implementation lives in @legendapp/frame-desktop-host/windows/runtimes.inc.\n");
  ...
  pkg.frame = { sdk: true, upstreamRevision: runtimesRevision, patchHash: hash };

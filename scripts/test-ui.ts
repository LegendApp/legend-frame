...
try {
  ...
  const log = Bun.file(path.join(root, ".spark/ui-metro.log"));
  ...
  const output = Bun.file(path.join(root, ".spark/ui-app.log"));
  app = Bun.spawn([path.join(product.app, "Contents/MacOS", executable), "-RCT_jsLocation", `127.0.0.1:${port}`, "--spark-ui-report", report], {

...
const root = path.resolve(process.argv[2] ?? ".legend/ui-tests/KitchenSink");
...
writeFileSync(driverFile, 'import driver from "@legend-apps/sdk-test-driver";\nexport type TestDriver = typeof driver;\nexport const testDriver = driver;\n');
...
try {
  ...
  const log = Bun.file(path.join(root, ".legend/ui-metro.log"));
  ...
  const report = path.join(root, `.legend/ui-results-${Date.now()}.json`);
  const output = Bun.file(path.join(root, ".legend/ui-app.log"));
  app = Bun.spawn([path.join(product.app, "Contents/MacOS", executable), "-RCT_jsLocation", `127.0.0.1:${port}`, "--legend-ui-report", report], {
    cwd: root, env: { ...process.env, LEGEND_BUNDLE_URL: `http://127.0.0.1:${port}/index.bundle?platform=macos&dev=true&minify=false` }, stdout: output, stderr: output,
  });

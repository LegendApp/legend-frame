...
try {
  if (!prepareOnly && ((platform === "windows" && process.platform !== "win32") || (["macos", "ios"].includes(platform) && process.platform !== "darwin"))) {
    ...
    } else if (platform !== "web") await run(root, [process.execPath, "node_modules/@legendapp/frame/bin/frame.cjs", "prebuild", "--platform", platform], { capture: true });
    ...
    if (!prepareOnly) {
      ...
      if (desktop) {
        ...
        if (platform === "windows") app = spawnProcess([path.join(product.app, "MyApp.exe")], { cwd: product.app, env: { ...process.env, ...projectEnvironment(root), FRAME_METRO_PORT: String(port) }, stdout: "inherit", stderr: "inherit" });
        else {
          ...
          app = spawnProcess([path.join(product.app, "Contents/MacOS", executable), "-RCT_jsLocation", `127.0.0.1:${port}`], { cwd: root, env: { ...process.env, ...projectEnvironment(root), FRAME_BUNDLE_URL: `http://127.0.0.1:${port}/index.bundle?platform=macos&dev=true&minify=false` }, stdout: "inherit", stderr: "inherit" });
        }

...
const report=path.join(framework,'.legend/fabric-reload-report.json');
const executable=root+'/.legend/platforms/macos/products/dev/KitchenSink.app/Contents/MacOS/KitchenSink';
if(!existsSync(executable))throw Error('Build Kitchen Sink first: bun run legend build --dev --project examples/kitchen-sink');
if(existsSync(entry))throw Error('Temporary entry already exists');
rmSync(report,{force:true});rmSync(report+'.count',{force:true});
writeFileSync(entry,`import {registerRootComponent} from 'expo'; import {useEffect} from 'react'; import {Text,DevSettings} from 'react-native'; import * as files from '@legend-apps/desktop/files'; import * as windows from '@legend-apps/desktop/windows';
...
const port=await availablePort();const log=Bun.file(path.join(framework,'.legend/fabric-reload-metro.log'));
const metro=Bun.spawn(nodeCommand(root,'expo','expo',['start','--localhost','--port',String(port),'--max-workers','2']),{cwd:root,env:{...process.env,CI:'1',LEGEND_PLATFORM:'macos'},stdout:log,stderr:log});let app;

...
import { windowsArchitecture } from "../packages/cli/src/platform";
...
const root = path.resolve(process.argv[2] ?? ".spark/examples/HelperDemo");
...
const pkg = readJson(path.join(root, "package.json")); pkg.dependencies["base64-js"] = "1.5.1"; pkg.dependencies["@legendapp/spark-ui"] = pkg.overrides["@legendapp/spark-ui"]; writeJson(path.join(root, "package.json"), pkg);
...
console.log(`Ready: ${root}\nRun bun run ${platform} there, then choose Build for the custom helper runtime.`);

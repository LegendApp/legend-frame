...
for (const example of ["notes-lite", "music-lite", "diff-lite"] as Example[]) {
  ...
  const directory = path.join(root, ".frame/checks"); mkdirSync(directory, { recursive: true });
  const original = readFileSync(path.join(root, "desktop.config.json"), "utf8");
  for (const platform of ["web", "ios", "android", "macos", "windows"]) {
    await run(root, nodeCommand(root, "expo", "expo", ["export:embed", "--entry-file", "index.ts", "--platform", platform, "--dev", "true", "--max-workers", "2", "--bundle-output", path.join(directory, `${platform}.js`), "--sourcemap-output", path.join(directory, `${platform}.map`)]), { capture: true, env: { CI: "1", FRAME_PLATFORM: platform } });
    const sources: string[] = JSON.parse(readFileSync(path.join(directory, `${platform}.map`), "utf8")).sources;
    if (["ios", "android", "web"].includes(platform) && sources.some(source => /NativeDesktop|NativeFrameAudio/.test(source))) throw new Error(`${example}/${platform} imports a desktop binding`);

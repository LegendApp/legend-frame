...
const binary = path.resolve(process.argv[2] ?? "examples/kitchen-sink/.legend/platforms/macos/products/dev/KitchenSink.app");
...
const parent = mkdtempSync(path.join(os.tmpdir(), "legend-upstream-run-"));
const root = path.join(parent, "BinaryProbe"), shims = path.join(parent, "bin");
mkdirSync(root); mkdirSync(shims);
const output = path.join(framework, ".legend/expo-desktop-run-probe"); mkdirSync(output, { recursive: true });
...
writeFileSync(path.join(root, "app.json"), JSON.stringify({ expo: { name: "BinaryProbe", slug: "binary-probe", platforms: ["macos"], macos: { bundleIdentifier: "org.legend.binaryprobe" } } }));
...
writeFileSync(path.join(shims, "open"), '#!/usr/bin/env node\nrequire("node:fs").writeFileSync(process.env.LEGEND_PROBE_RECORD, JSON.stringify({args:process.argv.slice(2)}));\n', { mode: 0o755 });
...
for (const phase of ["javascript-only", "existing-project"] as const) {
  ...
  const child = Bun.spawn(args, { cwd: root, detached: true, env: { ...process.env, CI: "1", EXPO_NO_GIT_STATUS: "1", LEGEND_PROBE_RECORD: record,

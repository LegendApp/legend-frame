...
export async function packCameraPackages(names?: string[]) {
  ...
  const cache = path.join(framework, ".spark/camera-packages");
  ...
  for (const [name, info] of Object.entries(upstream)) {
    ...
    await run(stage, ["tar", "--exclude=.spark", "-czf", path.join(output, file), "."], { capture: true });
...
export async function installCameraPackages(root: string, probeOnly = false) {
  ...
  packages["@legendapp/spark-nitro-view-probe"] = immutable;

...
const cli = path.join(root, "node_modules/@legendapp/frame/bin/frame.cjs");
...
function startSession(extra: string[] = []) {
  const child = spawnProcess([process.execPath, cli, "dev", "--project", root, ...extra], { cwd: root, env: { ...process.env, FRAME_SESSION_TOKEN: token }, stdin: "pipe", stdout: "pipe", stderr: "pipe" });
...
try {
  ...
  if (prepareOnly) {
    ...
    stage("Build and register the Frame Runner with frame sdk build-runner");
...
`);
    ...
    if (!proof.hermes || proof.native.fingerprint !== go.runtime.fingerprint || proof.native.mode !== "go") throw new Error("Wrong Frame Runner or JavaScript engine");
    pass(proof);
  ...
  if (prepareOnly) {
    ...
    }, "frame dev did not reject the incompatible Frame Runner");
    pass(issues);

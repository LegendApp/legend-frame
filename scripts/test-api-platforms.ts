...
const root = path.resolve(process.argv[2] ?? ".frame/api-platforms");
if (existsSync(path.join(root, "package.json")) && !existsSync(path.join(root, ".frame-api-probe"))) throw new Error("Choose an empty directory for the API platform probe");
...
const archives = { "@legendapp/frame": localArchive(path.join(framework, "artifacts/packages", manifest["@legendapp/frame"])) };
...
for (const platform of ["ios", "android", "web"]) {
  ...
  if (sources.some(source => source.includes("NativeDesktop") || source.includes("desktop-app/src") || source.includes("FrameButtonNativeComponent"))) throw new Error(`${platform} bundle loads desktop native modules`);
  for (const name of ["clipboard", "secure-storage", "desktop-links"]) {
    if (!sources.some(source => source.includes(`@legendapp/frame-${name}/src/index.${platform}.ts`))) throw new Error(`${platform} did not select ${name}'s platform adapter`);
  }
  if (!sources.some(source => source.includes(`@legendapp/frame-ui/src/index.${platform}.tsx`))) throw new Error(`${platform} did not select the UI adapter`);
...
const mobileEnv = { FRAME_DESKTOP_AUTOLINK: "" };

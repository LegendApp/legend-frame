...
export async function packageCameraApp(app: string, report: string) {
  ...
  const kit = path.join(output, "FrameCamera-macOS-arm64");
  const archive = path.join(output, "frame-camera-macos-arm64-test-kit.zip");

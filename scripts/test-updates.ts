...
const root = path.resolve(process.argv[2] ?? ".legend/update-tests/UpdateProbe");
...
const report = path.join(root, ".legend/release-update-report.json");
let app: ReturnType<typeof Bun.spawn> | undefined;
try {
  const config = readJson(configFile);
  const { publicKey } = generateKeyPairSync("ed25519");
  const framework = config.expo ? config.expo.extra.legend : config;
...
import { getUpdateStatus, startUpdates } from "@legend-apps/desktop/updates";
...
export default function App() {
  ...
  if (!info.LSUIElement || !info.LegendMenuBarOnly || !info.SURequireSignedFeed || !info.SUVerifyUpdateBeforeExtraction) throw new Error("Release updater/menu-bar CNG settings missing");
  await run(root, ["codesign", "--verify", "--deep", "--strict", result.app], { capture: true });
  rmSync(report, { force: true });
  const log = Bun.file(path.join(root, ".legend/update-release.log"));
  ...
  if (!existsSync(report)) throw new Error(`Release update test timed out: ${root}/.legend/update-release.log`);

...
const config = async (platform?: string) => JSON.parse(await run(root, nodeCommand(root, "expo", "expo", ["config", "--json"]), { capture: true, env: { LEGEND_PLATFORM: platform ?? "", APP_ENV: "integration" } }));
...
assert.equal(pkg.scripts["legend:macos"], "legend dev --platform macos");
...
for (const platform of ["macos", "windows"]) {
  ...
  assert.equal(value.extra.legend.projectId, readJson(path.join(root, "desktop.config.json")).projectId);
  await run(root, ["node", "-e", `const assert = require('node:assert/strict'); const config = require('./metro.config'); const rewritten = config.server.rewriteRequestUrl('/index${platform === "windows" ? ".windows" : ""}.bundle?platform=${platform}&dev=true'); assert.ok(rewritten.startsWith('/src/bootstrap.bundle?'), rewritten); assert.ok(config.resolver.sourceExts.includes('custom'));`], { capture: true, env: { LEGEND_PLATFORM: platform } });
}
await run(root, ["node", "-e", `const assert = require('node:assert/strict'); const { readConfig } = require('@legend-apps/desktop-config/config.cjs'); const a = readConfig(${JSON.stringify(root)}, 'macos'); const b = readConfig(${JSON.stringify(realpathSync(root))}, 'macos'); assert.deepEqual(a, b); assert.equal(a.expo._internal, undefined);`], { capture: true });
...
const output = path.join(root, ".legend/adoption-checks"); mkdirSync(output, { recursive: true });
for (const platform of ["ios", "android", "web", "macos", "windows"]) {
  const map = path.join(output, `${platform}.map`);
  await run(root, nodeCommand(root, "expo", "expo", ["export:embed", "--entry-file", pkg.main, "--platform", platform, "--dev", "true", "--max-workers", "2", "--bundle-output", path.join(output, `${platform}.js`), "--sourcemap-output", map]), { capture: true, env: { LEGEND_PLATFORM: platform, CI: "1" } });
...
await run(root, ["bun", "node_modules/@legend-apps/cli/src/index.ts", "prebuild", "--platform", "windows"], { capture: true, env: { CI: "1" } });
...
await run(simple, ["node", "-e", "const assert = require('node:assert/strict'); const c = require('./metro.config'); assert.ok(c.server.rewriteRequestUrl('/index.bundle?platform=macos&dev=true').includes('/node_modules/expo/AppEntry.bundle?'));"], { capture: true, env: { LEGEND_PLATFORM: "macos" } });
await run(simple, nodeCommand(simple, "expo", "expo", ["export:embed", "--entry-file", "node_modules/expo/AppEntry.js", "--platform", "macos", "--dev", "true", "--max-workers", "2", "--bundle-output", path.join(simple, ".legend/static.js")]), { capture: true, env: { LEGEND_PLATFORM: "macos", CI: "1" } });

...
const root = path.resolve(process.argv[2] ?? ".legend/runtimes-probe/LegendRuntimesProbe");
...
const directory = path.join(root, ".legend/runtimes-proof"); mkdirSync(directory, { recursive: true });
...
try {
  ...
  app = Bun.spawn([path.join(result.app, "Contents/MacOS", executable), "-RCT_jsLocation", `127.0.0.1:${port}`, "--legend-runtimes-report", report, ...(mode === "dev" ? ["--legend-runtimes-reload"] : [])], { cwd: root, env: { ...process.env, ...projectEnvironment(root), LEGEND_BUNDLE_URL: `http://127.0.0.1:${port}/index.bundle?platform=macos&dev=true&minify=false` }, stdout: log, stderr: log });

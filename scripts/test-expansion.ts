...
const root = path.resolve(process.argv[2] ?? ".legend/expansion-tests/Expansion");
...
const directory = path.join(root, ".legend/expansion-results"); mkdirSync(directory, { recursive: true });
...
try {
  writeJson(canonical, { name: previous.name, version: previous.version, projectId: previous.extra?.legend?.projectId ?? previous.macos.bundleIdentifier,
  ...
  for (const mode of ["dev", "go"] as const) {
    ...
    if (mode === "dev") pkg.dependencies["@legend-apps/sdk-test-driver"] = `file:${path.resolve(import.meta.dir, "../fixtures/sdk-test-driver")}`;
    else delete pkg.dependencies["@legend-apps/sdk-test-driver"];
    ...
    app = Bun.spawn([path.join(result.app, "Contents/MacOS", name), "-RCT_jsLocation", `127.0.0.1:${port}`, "--legend-expansion-report", report, "--legend-window-config-probe"], { cwd: root, env: { ...process.env, ...projectEnvironment(root), LEGEND_BUNDLE_URL: `http://127.0.0.1:${port}/index.bundle?platform=macos&dev=true&minify=false` }, stdout: appLog, stderr: appLog });
  ...
  if (pkg.dependencies["@legend-apps/sdk-test-driver"]) {
    delete pkg.dependencies["@legend-apps/sdk-test-driver"]; writeJson(pkgFile, pkg);
    await run(root, ["bun", "install"]);
  }
